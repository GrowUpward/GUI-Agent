...
错误样例显示，模型经常先生成一个完整且可执行的动作字典，随后继续生成新的 `assistant` 回合。部署端已经通过 `truncate_at_balanced_dict()` 只执行第一个完整动作，但原离线 SFT 评测直接解析全部文本，导致首个正确动作也可能被贪婪正则合并为畸形字典。

提交 `4f90b78` 将离线评测与部署协议对齐：解码后只保留第一个括号平衡的动作字典，再交给原有严格解析器。该修改不改变 checkpoint、数据、gold 标签或 reward 规则。

## 9. 单动作协议重评结果

使用相同 E1/E2 checkpoint，在相同 457 条 Validation 上双卡并行重评；manifest SHA256 仍为 `7c9a61c1f6fd42a89849281b12861f22daab63c842d9545b63491b6908b19fd7`，未读取 Test。E1 耗时 2022.2 秒，E2 耗时 1916.5 秒。

评测目录：

```text
artifacts/eval/e1-vs-e2-validation457-single-action-20260905-154959
```

| 指标 | E1 | E2 | 差值（E2 - E1） |
|---|---:|---:|---:|
| 解析成功率 | 100.00% | 100.00% | 0.00 pp |
| 格式有效率 | 100.00% | 100.00% | 0.00 pp |
| 动作类型准确率 | **83.15%** | 79.65% | -3.50 pp |
| 参数准确率 | 12.04% | **15.32%** | +3.28 pp |
| 平均 reward | **0.5254** | 0.4623 | -0.0631 |
| Click 命中率 | **25.91%** | 22.26% | -3.65 pp |
| Click 平均距离（越低越好） | 281.80 | **280.26** | -1.54 |
| Input text 完全匹配率 | 44.44% | **57.41%** | +12.97 pp |
| 分动作宏平均准确率 | 36.95% | **40.36%** | +3.41 pp |

相对修复前，E1/E2 的解析成功率均提高 11.16 个百分点；动作类型准确率分别提高 10.06 和 11.16 个百分点，平均 reward 分别提高 0.1241 和 0.1421。这证明原评测确实低估了两个模型，也说明离线评测必须与真机的“每轮只执行一个动作”协议保持一致。

修复后的分动作结果：

| Gold 动作 | 样本数 | E1 | E2 | 差值（E2 - E1） |
|---|---:|---:|---:|---:|
| Click | 328 | **94.21%** | 84.15% | -10.06 pp |
| Input text | 54 | 74.07% | **90.74%** | +16.67 pp |
| Stop | 58 | 53.45% | **67.24%** | +13.79 pp |
| Scroll | 11 | 0.00% | 0.00% | 0.00 pp |
| Wait | 4 | 0.00% | 0.00% | 0.00 pp |
| Impossible | 2 | 0.00% | 0.00% | 0.00 pp |

E2 并非全面退化：UI-R1 预热显著改善了 Input text 和 Stop，但 Click 动作准确率下降 10.06 个百分点。由于 Click 占 Validation 的 71.8%，该退化仍使 E2 的总体动作准确率和平均 reward 低于 E1。

## 10. 当前结论与决策规则

结论：**不采用 E2 作为下一阶段主基线，继续保留 E1。**

UI-R1 Stage 0 预热虽然将最终 Validation loss 降低约 0.56%，并改善 Input text 与 Stop，但没有转化为更好的整体生成行为：在与部署一致的单动作协议下，动作类型准确率仍下降 3.50 个百分点，平均 reward 下降 0.0631，Click 命中率下降 3.65 个百分点。因此，本实验不能将更低的 loss 表述为整体能力提升。

这次实验仍然有价值：它形成了一个更细致的消融结果，证明直接顺序微调会改变动作先验，并暴露出当前系统的三个关键问题：Click 占绝对多数、`scroll/wait/impossible` 完全没有学会、不同动作能力之间存在明显权衡。与此同时，单动作协议修复消除了 11.16% 的伪解析失败。

下一步仍以 E1 为总体基线，将 E2 在 Input text 和 Stop 上的提升作为可利用信号，针对动作不均衡设计采样或 loss 权重消融；目标是在保住 E1 Click 能力的同时补齐稀有动作。每次只改变一个变量，并继续只在固定 Validation 上选择方案。Test 保持封存，待方案固定后只运行一次，随后进入真机多步任务评测。

...
训练动作的完整字段、参数约束、CAGUI 映射和数据分布以 [训练动作空间协议](TRAINING-ACTION-SPACE.md) 为准。当前模型支持以下结构化动作：

- `click`：点击归一化坐标；
- `long_press`：在归一化坐标持续按压指定毫秒数；
- `scroll`：向上、下、左或右滚动；
- `input_text`：输入指定文本；
...
CAGUI 的 `NO_ACTION` 映射为固定 500ms wait。`LONG_POINT` 保留为独立动作，坐标和 duration 均参与监督。

模型输出采用单个 Python 字典形式，例如：

```python
{'action': 'click', 'coordinate': [520, 310]}
{'action': 'long_press', 'coordinate': [480, 320], 'duration': 1000}

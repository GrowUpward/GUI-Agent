...
第 5 step 时平均速度约为 19.2 秒/step，loss 为 0.8624、梯度范数为 4.53；两卡显存采样约为 18–19 GB，未出现 NaN 或 OOM。300 step 训练耗时 1 小时 52 分钟，最终 train loss 为 0.5823，Validation loss 从 step 100 的 0.5724 降至 step 300 的 0.5370。

### 9.1 固定 Test-439：Base 与 SFT 对照

使用同一版本化 manifest、完整 439 条 Test 样本、相同 prompt、448 图像边长和确定性解码，对未微调 Qwen3.5-0.8B 与 checkpoint-300 进行独立评测。结果目录为：

```text
artifacts/eval/base-vs-sft-test439-20260903-152122
```

| 指标 | Base | SFT checkpoint-300 | 变化 |
| --- | ---: | ---: | ---: |
| 格式/解析成功率 | 52.62% | 90.43% | +37.81 pp |
| 动作类型准确率 | 26.88% | 75.63% | +48.75 pp |
| 动作参数准确率 | 0.00% | 12.07% | +12.07 pp |
| 点击命中率 | 2.88% | 21.47% | +18.59 pp |
| 点击平均距离 | 427.75 | 280.53 | -147.22 |
| 输入文本完全匹配率 | 0.00% | 42.86% | +42.86 pp |
| 平均奖励 | -0.2218 | 0.4301 | +0.6519 |

动作类型分类中，SFT 的 Click 为 85.58%、Input text 为 64.29%、Stop 为 48.33%，但 Scroll 和 Wait 均为 0%。Base 的 Scroll 为 50%（4/8），SFT 在该小样本类别上退化为 0/8。Test 中 Click 占 312/439，即单纯预测 Click 的多数类准确率为 71.07%；因此 SFT 的总体动作类型准确率虽然比 Base 大幅提升，但仅比多数类基线高 4.56 pp。

结论：修正坐标后的 SFT 明确提升了格式遵循、主要动作识别、文本输入和点击定位，证明训练有效；但 21.47% 的点击命中率和长尾动作 0% 仍不足以作为最终模型结果。后续实验应优先解决坐标 grounding 与类别不平衡，而不是仅报告总体动作准确率。
